...
import { 
  ...
  CheckCircle2, 
  User,
  Loader2 // Hozzáadva a Modern mentési állapothoz
} from 'lucide-react';
...
interface SignatureProps {
  ...
  onHome?: () => void; // ✅ JAVÍTÁS: Home gomb prop hozzáadva
}
...
export function Signature({
  ...
  onBack,
  onComplete,
  onHome, // ✅ JAVÍTÁS: Home gomb prop fogadása
}: SignatureProps) {
  ...
  const [isSaving, setIsSaving] = useState(false); // Csak a Modern nézet használja

  const canComplete = true; // Engedélyezzük a befejezést aláírással vagy anélkül

  // === KÖZPONTI MENTŐ FÜGGVÉNY ===
  // (Különböző logikával a classic és modern nézethez)
  const handleSave = () => {
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-xl font-bold bg-gradient-to-r from-blue-600 via-sky-600 to-cyan-500 bg-clip-text text-transparent">
                    {/* ✅ JAVÍTÁS: Fordítás t.title-re */}
                    {t.title || 'OTIS APROD - Átvételi Protokoll'}
                  </h1>
                  <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1">
                    <Sparkles className="h-3 w-3 text-cyan-500" />
                    {/* ✅ JAVÍTÁS: Fordítás (hiányzó kulcs fallback) */}
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1 mt-1">
                      ...
                      {/* ✅ JAVÍTÁS: Visszacserélve a helyes kulcsra */}
