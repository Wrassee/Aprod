// OTIS APROD Service Worker v0.4.9 - Complete PWA Support
const CACHE_NAME = 'otis-aprod-v0.4.9';
...
console.log('[SW] OTIS APROD Service Worker v0.4.9 initializing');

// Install event - cache essential resources
self.addEventListener('install', (event) => {
  console.log('[SW] Installing Service Worker v0.4.9');
  
  event.waitUntil(
...
  console.log('[SW] Activating Service Worker v0.4.9');
  
  event.waitUntil(
    caches.keys()
      .then((cacheNames) => {
...
      })
      .then(() => {
        console.log('[SW] Service Worker v0.4.9 activated');
...
  }
  ...
  // Hagyja figyelmen kívül az API és Auth hívásokat.
  // Ez kritikus az /auth/callback és a /api hívásokhoz.
  if (url.pathname.startsWith('/api') || url.pathname.startsWith('/auth')) {
    console.log('[SW] Ignoring auth/api request, letting browser handle:', url.pathname);
    return; // Kilépés az SW logikából, a böngésző kezeli a kérést
  }
...
  G     // Protocol data is already in localStorage and will be available when online
    }
  } catch (error) {
